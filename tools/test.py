...
# custom imports to register modules in mmpose
from dataset import SpineTrackDataset  # noqa: F401
from metric import SpineTrackMetric  # noqa: F401


def parse_args():
    parser = argparse.ArgumentParser(description="MMPose test (and eval) model")
    ...
    parser.add_argument(
        "--badcase", action="store_true", help="whether analyze badcase in test"
    )
...
def merge_args(cfg, args):
    ...
    if (args.show and not args.badcase) or (args.show_dir is not None):
        ...
        cfg.default_hooks.visualization.show = False if args.badcase else args.show
        if args.show:
    ...
    if args.badcase:
        ...
        if metric_type not in ["loss", "accuracy"]:
            raise ValueError(
                "Only support badcase metric type" "in ['loss', 'accuracy']"
            )
